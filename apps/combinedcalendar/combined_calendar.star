...
CLEAR_DAY_PNG = base64.decode("")
...
def main(config):
    ...
    if location_str and location_str.startswith("{"):
        ...
        if loc != None and loc.get("timezone"):
            tz = loc.get("timezone")
    ...
    for i in range(4):
        number = str(i + 1)
        url = config.get("calendar_" + number + "_url")
        if not url:
            continue
        configured = True
        name = config.get("calendar_" + number + "_name") or CALENDAR_DEFAULTS[i][0]
        color = config.get("calendar_" + number + "_color") or CALENDAR_DEFAULTS[i][1]
        for event in fetch_events(tz, url, now):
            event["calendar"] = name
            event["color"] = color
            events.append(event)
...
def calendar_fields(number, default_name, default_color):
    suffix = str(number)
    return [
        schema.Text(
            id = "calendar_" + suffix + "_name",
            name = "Calendar " + suffix + " Name",
            desc = "A short label used to identify this calendar.",
            icon = "tag",
            default = default_name,
        ),
        schema.Color(
            id = "calendar_" + suffix + "_color",
            name = "Calendar " + suffix + " Color",
            desc = "Color used for events from this calendar.",
            icon = "brush",
            default = default_color,
        ),
        schema.Text(
            id = "calendar_" + suffix + "_url",
            name = "Calendar " + suffix + " iCal URL",
            desc = "Google Calendar secret address in iCal format. Leave blank to skip this calendar.",
            icon = "calendar",
            secret = True,
        ),
    ]
...
def get_schema():
    fields = []
    for i in range(4):
        fields.extend(calendar_fields(i + 1, CALENDAR_DEFAULTS[i][0], CALENDAR_DEFAULTS[i][1]))
    fields.extend([
        schema.Toggle(
            id = "include_all_day",
        ...
        schema.Location(
            id = "location",
            name = "Location",
            desc = "Used to show events in your local timezone.",
            icon = "locationDot",
        ),
    ])
    return schema.Schema(version = "1", fields = fields)

def select_today(events, now, include_all_day):
    chosen = []
    seen = {}
    for event in events:
        all_day = event["all_day"]
        is_today = same_day(event["start"], now)
        active_today = not all_day and event["start"] < now and event["end"] > now
        future_today = not all_day and is_today and event["start"] >= now
        if not ((include_all_day and all_day and is_today) or active_today or future_today):
            continue
        key = event["summary"] + "|" + str(event["start"].unix) + "|" + str(event["end"].unix)
        if key in seen:
            continue
        seen[key] = True
        insert_event(chosen, event)
    return chosen

def event_rank(event):
    return -1 if event["all_day"] else event["start"].unix

def insert_event(events, event):
    rank = event_rank(event)
    for i in range(len(events)):
        if rank < event_rank(events[i]):
            events.insert(i, event)
            return
    events.append(event)

def format_compact_time(event, now):
    if event["all_day"]:
        return "ALL"
    if event["start"] <= now and event["end"] > now:
        return "NOW"
    hour = event["start"].hour % 12
    if hour == 0:
        hour = 12
    minute = str(event["start"].minute)
    if event["start"].minute < 10:
        minute = "0" + minute
...
def agenda_row(event, now, scale, scroll_horizontally = True):
    ...
    row = render.Row(
        ...
        children = [
            render.Text(
                content = format_compact_time(event, now),
                color = event["color"],
                font = font,
            ),
    ...
    return render.Root(
        child = render.Box(
            ...
            child = render.Column(
                ...
                cross_align = "center",
                children = [
                    render.Text(content = line1, color = "#FFFFFF", font = font),
                    render.Text(content = line2, color = "#888888", font = font),
                ],
            ),
        ),
        max_age = 60,
